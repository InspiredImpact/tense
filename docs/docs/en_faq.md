...
# What is tense?
`tense` - time parsing tool. Did you need to convert the string 
...
# How is tense designed?
`tense` designed to provide maximum **flexibility**, **practicality** and **convenience** to the user.
...
assert parser.parse(time_string) == 86480  # It works.
```
...
[See to github](https://github.com/Animatea/tense/blob/main/tense/application/factory.py)

The first argument to `TenseParser` is a subclass of *tense.application.ports.parsers.AbstractParser*.
Initially, there are already some ready-made implementations, such as **TenseParser.DIGIT** - tense.adapters.parsers.DigitParser
and **TIMEDELTA** - tense.adapters.parsers.TimedeltaParser.

!!! note
    You can also create your own custom parsers inheriting **tense.application.ports.parsers.AbstractParser**
...
[See to github](https://github.com/Animatea/tense/blob/main/tense/application/resolvers.py)
...
!!! note
    We try to make `tense` as flexible as possible, so almost every hook you can
    implement in your own way and transfer to the appropriate place. Converter is no exception, you
    you can create your own custom one by first deriving it from tense.application.ports.converters.AbstractConverter.
...
Atomic operations in `tense` are implemented by the **UnitOfWork** pattern, which in turn interacts with
alias repository.
...
!!! info "Let's take a closer look"
    ...
    with unit_of_work.TenseUnitOfWork() as uow:
        ...
        assert "second" in uow.products.get_setting(second_str, "aliases")  # Default "second" alias in units.Second aliases
    
        uow.delete_aliases(units.Second, ["second"])  # Remove alias from units.Second
    
        assert "second" not in uow.products.get_setting(second_str, "aliases")  # Alias has been removed.
    ```
    === "Replacing aliases"
    ```py
    from tense import unit_of_work, units
    ...
    with unit_of_work.TenseUnitOfWork() as uow:
        ...
        alias_state = uow.products.get_setting(second_str, "aliases")  # Get alias state after replacement
    ...
    __Next, we pass the repository sources to TenseParser(tenses=uow.products.config)__

# How to add custom time units?
For custom time units, tense.domain.units has a separate class - VirtualUnit. He is no different
...
assert parser.parse("1 twoseconds 1 threeseconds") == 5
```
...
The `tense` config is not tied to a specific file type or file name, but we recommend sticking to the basic
standard - the name ".tense".

The tense config file parser supports just a few patterns, but you don't need more. let's consider
each of them.
...
=== "ListValueConverter"
    !!! danger
        Lists in `tense` are similar in pattern to python tuples. If you have 
...
[See on github](https://github.com/Animatea/tense/blob/main/tense/service_layer/dot_tense/step_chain.py)
...
??? info "Open base configuration template"
    [See on github](https://github.com/Animatea/tense/blob/main/tense/adapters/repository.py)
    ```json
